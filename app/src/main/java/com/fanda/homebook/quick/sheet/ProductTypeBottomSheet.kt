...
@Composable fun ProductTypeBottomSheet(product: String, showBottomSheet: Boolean, onDismiss: () -> Unit, onConfirm: (String) -> Unit) {
    CustomBottomSheet(visible = showBottomSheet, onDismiss = onDismiss) {
        var selected by remember { mutableStateOf(product) }
        Column(
            ...
            Box(
                ...
                Text(
                    modifier = Modifier.align(Alignment.Center), style = TextStyle.Default, text = "颜色", color = Color.Black, fontWeight = FontWeight.Medium, fontSize = 16.sp
                )
            ...
            LazyColumn(
                ...
                items(LocalDataSource.productData, key = { it }) {
                    Row(
