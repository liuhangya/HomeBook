...
@Composable fun EditProductPage(modifier: Modifier = Modifier, navController: NavController, productViewModel: ProductViewModel = viewModel(factory = AppViewModelProvider.factory)) {
    ...
    Scaffold(modifier = modifier.statusBarsPadding(), topBar = {
        TopIconAppBar(
            ...
            onRightActionClick = {
                productViewModel.toggleAddDialog(true)
            },
            backIconPainter = painterResource(R.mipmap.icon_back),
        )
    }) { padding ->
        ProductDragWidget(Modifier.padding(padding), data = products, onEditClick = {
            productViewModel.updateEntity(it)
    ...
    if (uiState.editDialog) {
        EditDialog(title = "重命名", value = uiState.entity?.name ?: "", placeholder = "不能与已有名称重复", onDismissRequest = {
            ...
            productViewModel.toggleEditDialog(false)
            productViewModel.updateEntityDatabase(it)
        })
    }

    if (uiState.addDialog) {
        EditDialog(title = "添加", value = "", placeholder = "不能与已有名称重复", onDismissRequest = {
            productViewModel.toggleAddDialog(false)
        }, onConfirm = {
            productViewModel.insertWithAutoOrder(it)
        })
    }
    RenameOrDeleteBottomSheet(visible = uiState.renameOrDeleteBottomSheet, onDismiss = {
        ...
        if (it == RenameOrDeleteType.RENAME) {
            productViewModel.toggleEditDialog(true)
        } else {
            productViewModel.deleteEntityDatabase()
        }
    }
}
...
@Composable @Preview(showBackground = true) fun ProductDragWidgetPreview() {
    ProductDragWidget(modifier = Modifier
        .fillMaxWidth()
        .statusBarsPadding(), data = defaultProductData, onEditClick = {}, onMove = { _, _, _ -> })
}
//
