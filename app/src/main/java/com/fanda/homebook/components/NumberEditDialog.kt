 ...
 * @param showSuffix 是否显示字符计数后缀，默认为false
 */
@Composable fun NumberEditDialog(
    title: String,
            ...
            Column(
                ...
                Row(
                    ...
                    SelectableRoundedButton(
                        text = "确定",                 // 按钮文本
                        selected = true,              // 选中状态
                        onClick = { onConfirm(newValue) }, // 点击触发确认
                        cornerSize = 27.dp,           // 圆角大小
                        contentPadding = PaddingValues(
                            horizontal = 47.dp,       // 水平内边距
                            vertical = 15.dp          // 垂直内边距
...
/**
 * 预览函数，用于在Android Studio中预览数字编辑对话框
 * 注意：这里错误地调用了EditDialog而不是NumberEditDialog，应修正为NumberEditDialog
 */
@Composable @Preview(showBackground = true) fun NumberEditDialogPreview() {
    HomeBookTheme {
        NumberEditDialog(
