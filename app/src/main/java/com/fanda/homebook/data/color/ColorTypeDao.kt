...
@Dao
interface ColorTypeDao {

    // 获取数据总数
    @Query("SELECT COUNT(*) FROM color_type")
    suspend fun getCount(): Int

    // 批量插入
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertAll(colorTypes: List<ColorTypeEntity>)
    ...
    @Query("DELETE FROM color_type WHERE id = :id")
    suspend fun deleteById(id: Int): Int

    // Flow 版本，支持实时更新，按排序字段升序
    @Query("SELECT * FROM color_type ORDER BY sortOrder ASC")
    fun getColorTypes(): Flow<List<ColorTypeEntity>>

    @Query("SELECT * FROM color_type WHERE id = :id")
    fun getColorTypeById(id: Int): Flow<ColorTypeEntity>

    // 获取最大排序值
    @Query("SELECT MAX(sortOrder) FROM color_type")
    ...
    suspend fun insertWithAutoOrder(colorType: ColorTypeEntity): Long {
        ...
        val insertEntity = colorType.copy(sortOrder = maxOrder + 1)
